...
class Dwoo_Compiler implements Dwoo_ICompiler
{
	...
	protected function parse($in, $from, $to, $parsingParams = false, $curBlock='', &$pointer = null)
	{
		...
		if ($first==='$') {
			...
			// parse error
			$szLine = substr($in, $from, $to-$from);
//			$szLine = strtok(substr($in, $from, $to-$from), "\r\n");
			$szLine = htmlspecialchars($szLine);
			throw new Dwoo_Compilation_Exception($this, 'Parse error in "'.$szLine.'"');
		}
