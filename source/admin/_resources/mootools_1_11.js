...
Element.extend({
// RUDIE's EDIT //
	firstParent : function(f_szNodeName) {
		var p = this.parentNode, nn = f_szNodeName.toUpperCase();
		while ( p && p.nodeName != nn ) {
			p = p.parentNode;
		}
		return p ? $(p) : p;
	},
	parent: function(tn) {
		return tn ? this.firstParent(tn) : $(this.parentNode);
	},
...
Element.extend({
	...
	getFormElements: function(){
		...
		return $combine(this.getElementsByTagName('input'), this.getElementsByTagName('select'), this.getElementsByTagName('textarea'));
	},
...
// RUDIE's EDIT //
		...
		if ( this.options.execGlobalHandlers && 'function' == typeof Ajax.onComplete ) {
			this.addEvent('onComplete', function(){ Ajax.busy--; Ajax.onComplete(); });
		}
// RUDIE's EDIT //
